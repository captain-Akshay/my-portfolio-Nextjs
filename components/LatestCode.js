    ...
    <section className="bg-inherit -mt-40 pb-40">
      <div className="max-w-6xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-center md:pt-40 mx-10">
          <h1
            className={`text-6xl lg:text-9xl max-w-lg font-bold text-gray-500 my-20 md:my-0  ${
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl mx-auto px-10 lg:-mt-10 gap-y-20">
        {repos &&
          repos.map((latestRepo, idx) => (
            <GithubRepoCard latestRepo={latestRepo} key="idx" isDark={isDark} />
          ))}
      </div>
    </section>
  );
...
const GithubRepoCard = ({ latestRepo, isDark }) => {
  return (
    <div className={`github-repo ${isDark ? "dark" : ""}`}>
      <h1
